...
class bts(nn.Module):
    def __init__(self, feat_out_channels, dataset, max_depth, num_features=512):
    ...
    def forward(self, features, focal):
        skip0, skip1, skip2, skip3 = features[1], features[2], features[3], features[4]     # 64, 256, 512, 1024 | H/2, H/4, H/8, H/16
        dense_features = torch.nn.ReLU()(features[5])   # B, 2048, H/32, W/32
        upconv5 = self.upconv5(dense_features)  # B, 512, H/16, W/16
        upconv5 = self.bn5(upconv5)
        concat5 = torch.cat([upconv5, skip3], dim=1)    # B, 1536, H/16, W/16
        iconv5 = self.conv5(concat5)    # B, 512, H/16, W/16

        upconv4 = self.upconv4(iconv5)  # H/8
        upconv4 = self.bn4(upconv4)     # B, 256, H/8, W/8
        concat4 = torch.cat([upconv4, skip2], dim=1)    # B, 768, H/8, W/8
        iconv4 = self.conv4(concat4)
        iconv4 = self.bn4_2(iconv4) # B, 256, H/8, W/8

        daspp_3 = self.daspp_3(iconv4)  # B, 128, H/8, W/8
        concat4_2 = torch.cat([concat4, daspp_3], dim=1)    # B, 896, H/8, W/8
        daspp_6 = self.daspp_6(concat4_2)   # B, 128, H/8, W/8
        concat4_3 = torch.cat([concat4_2, daspp_6], dim=1)  # B, 1024, H/8, W/8
        daspp_12 = self.daspp_12(concat4_3)     # B, 128, H/8, W/8
        concat4_4 = torch.cat([concat4_3, daspp_12], dim=1) # B, 1152, H/8, W/8
        daspp_18 = self.daspp_18(concat4_4)     # B, 128, H/8, W/8
        concat4_5 = torch.cat([concat4_4, daspp_18], dim=1)
        daspp_24 = self.daspp_24(concat4_5)
        concat4_daspp = torch.cat([iconv4, daspp_3, daspp_6, daspp_12, daspp_18, daspp_24], dim=1)  # B, 896, H/8, W/8
        daspp_feat = self.daspp_conv(concat4_daspp)     # B, 128, H/8, W/8

        reduc8x8 = self.reduc8x8(daspp_feat)    # B, 4, H/8, W/8
        ...
        depth_8x8 = self.lpg8x8(plane_eq_8x8, focal)    # B, H, W
        depth_8x8_scaled = depth_8x8.unsqueeze(1) / self.max_depth
        depth_8x8_scaled_ds = torch_nn_func.interpolate(depth_8x8_scaled, scale_factor=0.25, mode='nearest')    # B, 1, H/4, W/4

        upconv3 = self.upconv3(daspp_feat)  # H/4
        upconv3 = self.bn3(upconv3)     # B, 128, H/4, W/4
        concat3 = torch.cat([upconv3, skip1, depth_8x8_scaled_ds], dim=1)
        iconv3 = self.conv3(concat3)    # B, 128, H/4, W/4

        reduc4x4 = self.reduc4x4(iconv3)    # B, 4, H/4, W/4
        ...
        iconv2 = self.conv2(concat2)    # B, 64, H/2, W/2

        reduc2x2 = self.reduc2x2(iconv2)    # B, 4, H/2, W/2
        ...
        upconv1 = self.upconv1(iconv2)  # B, 32, H, W
        reduc1x1 = self.reduc1x1(upconv1)   # B, 1, H, W
        concat1 = torch.cat([upconv1, reduc1x1, depth_2x2_scaled, depth_4x4_scaled, depth_8x8_scaled], dim=1)   # B, 36, H, W
        iconv1 = self.conv1(concat1)    # B, 32, H, W
